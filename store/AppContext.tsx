...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const initDatabase = async () => {
      try {
        ...
        const agentsData = await supabaseService.getUsers();
        const pagesData = await supabaseService.getPages();
        const convsData = await supabaseService.getConversations();
        const msgsData = await supabaseService.getMessages();
        const linksData = await supabaseService.getApprovedLinks();
        const mediaData = await supabaseService.getApprovedMedia();

        // If no users in database, insert mock users
        if (agentsData.length === 0 && MOCK_USERS.length > 0) {
          for (const u of MOCK_USERS) {
            await supabaseService.saveUser(u);
          }
          setAgents(MOCK_USERS);
        } else {
    ...
    return () => {
      ...
      supabaseService.cleanupSubscriptions();
    };
  }, []);

  const setupRealtimeSubscriptions = () => {
    // Subscribe to conversation changes
    supabaseService.subscribeToConversations((payload) => {
    ...
    supabaseService.subscribeToMessages((payload) => {
  ...
  useEffect(() => {
    ...
    const deltaSync = async () => {
        ...
        try {
          ...
          for (const conv of metaConvs) {
            ...
            if (!local || local.lastTimestamp !== conv.lastTimestamp) {
              ...
              await supabaseService.saveConversation(conv);
      ...
      if (changesDetected) {
        const all = await supabaseService.getConversations();
  ...
  const syncMetaConversations = async () => {
    ...
    setDbStatus('syncing');
    try {
      const existingConvs = await supabaseService.getConversations();
      ...
      const allConvs = await supabaseService.getConversations();
  ...
  const clearLocalChats = async () => {
    setDbStatus('syncing');
    try {
      await supabaseService.clearStore('conversations');
      await supabaseService.clearStore('messages');
      setConversations([]);
      setMessages([]);
  ...
  const deleteConversation = async (id: string) => {
    setDbStatus('syncing');
    try {
      await supabaseService.delete('conversations', id);
      const allMsgs = await supabaseService.getMessages();
      for (const msg of allMsgs) {
        if (msg.conversationId === id) {
          await supabaseService.delete('messages', msg.id);
        }
      }
  ...
  const updatePage = async (id: string, updates: Partial<FacebookPage>) => {
    ...
    if (page) await supabaseService.savePage(page);
    setDbStatus('connected');
  };
  ...
  const addPage = async (page: FacebookPage) => {
    ...
    await supabaseService.savePage(page);
    setDbStatus('connected');
  };
  ...
  const removePage = async (id: string) => {
    ...
    await supabaseService.delete('facebook_pages', id);
    setDbStatus('connected');
  };
  ...
  const updateConversation = async (id: string, updates: Partial<Conversation>) => {
    ...
    if (conv) await supabaseService.saveConversation(conv);
  };
  ...
  const bulkAddMessages = async (msgs: Message[], silent: boolean = false) => {
    setMessages(prev => {
      ...
      supabaseService.saveMessages(newOnes);
  ...
  const simulateIncomingWebhook = async (pageId: string) => {
    ...
    await supabaseService.saveConversation(newConv);
  };
  ...
  return (
    <AppContext.Provider value={{
      ...
      agents, addAgent: async (a) => { setAgents(p => [...p, a]); await supabaseService.saveUser(a); },
      removeAgent,
      updateUser: async (id, u) => { 
        ...
        if (agent) {
          await supabaseService.saveUser(agent);
